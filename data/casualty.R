...
casualty_fun <- function(congress) {
  ...
  reasons <- map2(reasons, counts$V3, ~rep(.x, .y))
  legis$chamber <- flatten_chr(chamber)
  legis$reasons <- flatten_chr(reasons)
...
dat %<>%
  mutate(
    name = case_when(
      name == 'Russ Carnahan  D-Mo' & congress == 112 ~ 'Russ Carnahan',
      TRUE ~ name
    ),
    party = case_when(
      name == 'Russ Carnahan' & congress == 112 ~ 'D',
      name == 'Mark Pryor' & congress == 113 ~ 'D',
      name == 'Dave Camp' & congress == 113 ~ 'R',
      TRUE ~ party
    ),
    state = case_when(
      name == 'Russ Carnahan' & congress == 112 ~ 'Mo',
      name == 'Mark Pryor' & congress == 113 ~ 'Arkansas',
      name == 'Dave Camp' & congress == 113 ~ 'Mich.',
      TRUE ~ state
    )
  )

# standardize states
states <- dbGetQuery(db, "
  SELECT * FROM states
  ")
probs <- filter(dat, !state %in% states$state_name)$state
probs %<>% unique()
probs %<>% sort()
...
# opensecrets data
cands <- dbGetQuery(os, "
  SELECT DISTINCT cycle, osid, firstlast, distid_run
  FROM cands
  ") %>%
  mutate(
    name = str_trim(firstlast),
    name = str_replace(name, '\\([^()]*\\)$', ''),
    state_code = str_sub(distid_run, 1, 2)
  ) %>%
  mutate_if(is.character, str_trim) %>%
  distinct(cycle, name, osid, state_code)
